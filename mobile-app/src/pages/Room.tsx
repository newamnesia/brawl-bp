...
import { clearRoomSession, disconnectSocket, getSocket, type RoomState } from "../lib/socket";
import { invitationUrl } from '../lib/service';
...
export default function Room() {
  ...
  const copyInvite = async () => {
    try {
      if (Capacitor.isNativePlatform()) { await Share.share({ title: '加入 BP 房间', text: `房间号：${code}`, url: inviteUrl }); return; }
  ...
  useEffect(() => {
    const disconnected = () => { setState(null); setConnectTimedOut(false); };
    const connected = () => socket.emit('request_state');
    const resume = () => { if (!socket.connected) socket.connect(); else socket.emit('request_state'); };
    const failed = (event: Event) => { setClosedMsg((event as CustomEvent<string>).detail || '无法恢复房间'); setState(null); };
    const back = () => {
      if (window.confirm('退出当前 BP 房间？')) { clearRoomSession(); socket.emit('leave_room'); disconnectSocket(); navigate('/bp'); }
    };
    socket.on('disconnect', disconnected);
    socket.on('connect', connected);
    window.addEventListener('app-resume', resume);
    window.addEventListener('room-resume-failed', failed);
    window.addEventListener('room-back', back);
    ...
    return () => {
      socket.off('disconnect', disconnected); socket.off('connect', connected);
      window.removeEventListener('app-resume', resume); window.removeEventListener('room-resume-failed', failed); window.removeEventListener('room-back', back);
  ...
  if (!state) {
    return (
      <div className="app-shell">
        <p className="waiting-text">{connectTimedOut ? '连接较慢，请检查网络或重新加入。' : '正在连接或恢复房间，禁选倒计时仍在继续…'}</p>
        {(
