...
const MAGAZINE_RELOAD_SECONDS = 2.3;
// 走位训练射击节奏见 firing.ts，普通射击保留两发弹药。
const BEA_MAGAZINE_CAPACITY = 1;
const BEA_RELOAD_SECONDS = 0.9;
...
const STAR_RADIUS = 50 / TILE_SIZE;
const STAR_SPAWN_MIN_DISTANCE = 6;
const STAR_SPAWN_MAX_DISTANCE = 9;
...
export default function OfflineTrainingGame() {
  ...
  const mode = (searchParams.get("mode") as ControlMode) || "joystick";
  const speedTier: SpeedTier = searchParams.get("speedTier") === "high" ? "high" : "mid";
  ...
  useEffect(() => { roundId.current = crypto.randomUUID(); trainingSecondsRef.current = 0; setHistoryStatus(''); }, [restartNonce]);
  ...
  useEffect(() => {
    pausedRef.current = paused || roundResult !== null;
  ...
  useEffect(() => {
    const pause = () => {
      ...
      joystickRef.current.knobX = joystickRef.current.knobY = joystickRef.current.rawMagnitude = 0;
      aimJoystickRef.current.active = false;
      aimJoystickRef.current.touchId = null;
      aimJoystickRef.current.knobX = aimJoystickRef.current.knobY = aimJoystickRef.current.rawMagnitude = 0;
    };
    const visibility = () => { if (document.hidden) pause(); };
    window.addEventListener('training-pause', pause);
    window.addEventListener('blur', pause);
    document.addEventListener('visibilitychange', visibility);
    return () => { window.removeEventListener('training-pause', pause); window.removeEventListener('blur', pause); document.removeEventListener('visibilitychange', visibility); };
  }, []);

  useEffect(() => {
    if (!roundResult) return;
    setHistoryStatus('正在保存训练记录…');
    void saveTraining({ id: roundId.current, at: new Date().toISOString(), mode: isAimingMode ? '瞄准训练' : isSurvivalMode ? '走位挑战' : '走位训练', hero: isBeaMode ? '贝亚' : '佩佩', result: roundResult === 'victory' ? '胜利' : roundResult === 'defeat' ? '挑战结束' : '主动结束', duration: trainingSecondsRef.current, damage: totalDamageRef.current, reactionMs: mean(profilerRef.current?.samplesReactionMs ?? []), turnIntervalMs: mean(profilerRef.current?.samplesTurnIntervalMs ?? []), aimLeadDeg: mean(aimingLeadAnglesRef.current) }).then(() => setHistoryStatus('已保存到本机训练历史')).catch(() => setHistoryStatus('保存失败，请检查设备存储空间'));
  }, [roundResult]);
  ...
  useEffect(() => {
    ...
    const clearKeys = () => { keys.clear(); updateInput(); };
    window.addEventListener('blur', clearKeys);
    window.addEventListener('training-pause', clearKeys);
    return () => {
      window.removeEventListener('blur', clearKeys);
      window.removeEventListener('training-pause', clearKeys);
  ...
  useEffect(() => {
    ...
    const spawnStar = () => {
      ...
      for (let attempt = 0; attempt < 30; attempt++) {
        ...
        if (Math.hypot(x - collector.x, y - collector.y) < PLAYER_RADIUS + STAR_RADIUS + 0.35) continue;
        if (stars.some((star) => Math.hypot(x - star.x, y - star.y) < STAR_RADIUS * 2 + 0.35)) continue;
    ...
    const getInterceptPressure = (star: TrainingStar, now: number) => {
      ...
      const interceptRate = recent.length > 0
        ? recent.filter((shot) => rayDistanceToPoint(playerRef.current.x, playerRef.current.y, shot.angle, star.x, star.y) <= 0.75).length / recent.length
        : 0;
    ...
    const gameLoop = (now: number) => {
      ...
      if (!pausedRef.current) {
        ...
        playerIsMovingRef.current = !isAimingMode && curSpeed > 0.05;
        ...
        if (isAimingMode) {
          ...
          const rankedStars = stars.map((star) => {
            ...
            const projectileDanger = bulletsRef.current.filter((bullet) => bullet.owner === "player")
              .filter((bullet) => rayDistanceToPoint(bullet.x, bullet.y, Math.atan2(bullet.vy, bullet.vx), star.x, star.y) <= 0.75).length;
            const starDistanceFromShooter = Math.hypot(star.x - player.x, star.y - player.y);
            const boundaryPenalty = Math.max(0, starDistanceFromShooter - 9) ** 2;
            return {
              star,
          ...
          if (aiFeint) {
            ...
            if (!feintStar) {
              ...
              const reactedShot = [...playerShotHistoryRef.current].reverse().find((shot) =>
                ...
                && rayDistanceToPoint(player.x, player.y, shot.angle, feintStar.x, feintStar.y) <= 0.85);
              if (reactedShot) {
          ...
          if (threat) {
            ...
            ai.desiredHeading = evadeHeading;
          ...
          } else if (radialDistance >= 9.8) {
            ai.desiredHeading = Math.atan2(player.y - target.y, player.x - target.x);
          } else if (aiFeint?.phase === "break") {
            ai.desiredHeading = aiFeint.breakHeading;
            ...
            ai.desiredHeading = Math.atan2(selectedStar.y - target.y, selectedStar.x - target.x);
            ai.changeTimer = 0.10;
          } else if (!selectedStar && ai.changeTimer <= 0 && ai.dodgeLockTimer <= 0) {
            const centerX = player.x + Math.cos(AIMING_FRONT_ANGLE) * 8;
            const centerY = player.y + Math.sin(AIMING_FRONT_ANGLE) * 8;
            ai.desiredHeading = Math.atan2(centerY - target.y, centerX - target.x) + (Math.random() - 0.5) * 1.2;
          ...
          if (radialDistance > 9.4) {
            ...
            const boundaryStrength = Math.min(1, (radialDistance - 9.4) / 0.4);
          ...
          let nextX = target.x + aiMoveX * MOVE_SPEED * dt;
          let nextY = target.y + aiMoveY * MOVE_SPEED * dt;
      ...
      drawUnitStatusBars(ctx, {
        ...
        width: scale * widthFactorAt(renderedEnemy.y),
  ...
  const handleAimPointerUp = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (e.type !== 'pointercancel' && !pausedRef.current && directionLength > 8 && magazineAmmoRef.current > 0) {
      const player = playerRef.current;
      const target = aimingTargetRef.current;
    ...
    >
      ...
      >
        ...
        <div className="training-hud-actions" style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
          ...
          >
            {Capacitor.isNativePlatform() ? '横屏训练' : isFullscreen ? "▣ 退出全屏" : "⛶ 全屏"}
          </button>
          <button
            onClick={togglePause}
            style={{
  ...
  if (aiming) {
    return (
      <div className="training-chart-grid training-aiming-stats-grid">
        ...
        <div className="training-ratio-card">
          <div className="training-ratio-title">数据3 · 命中率</div>
