  ...
  return (
    <div className="app-shell solo-bp-setup">
      ...
      <section className={`card solo-setup-section ${!gameMode ? "is-locked" : ""}`}>
        <h2>2. 选择地图</h2>
        {!gameMode ? (
          <p className="solo-setup-hint">请先选择游戏模式。</p>
          ...
          <div className="solo-map-grid">
            {availableMaps.map((map) => (
              <button
                key={map.id}
                className={`solo-map-card ${mapId === map.id ? "selected" : ""}`}
                onClick={() => {
                  setMapId(map.id);
                  setFirstPicker(null);
                }}
      ...
      <section className={`card solo-setup-section ${!mapId ? "is-locked" : ""}`}>
        <h2>3. 选择先手方</h2>
        {!mapId ? (
          <p className="solo-setup-hint">请先选择地图。</p>
        ) : (
      ...
      <button
        className="btn-primary solo-start-button"
        disabled={!canStart}
        onClick={() => navigate(`/solo-bp/board?mode=${gameMode}&map=${mapId}&first=${firstPicker}`)}
      >
        开始
      </button>
    </div>
